...
test.describe("Patients Listing Page", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Patients Dashboard" })
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          patientsHeading &&
          patientsHeading.textContent === "Patients Dashboard" &&
          !loadingSpinner
        );
      },
      { timeout: 90000 }
  ...
  test("should display the patients dashboard header and description", async ({
    page,
  }) => {
    await expect(
      page.getByRole("heading", { name: "Patients Dashboard" })
    ...
    await expect(
      page.getByText(
        "Monitor and manage your patients' dementia risk assessments"
      )
    ).toBeVisible();
  });

  test("should display all stat cards", async ({ page }) => {
    const statCardTitles = [
      "Total Patients",
      "High Risk",
      "Moderate Risk",
      "Low Risk",
  ...
  test("should have a search input for filtering patients", async ({
    page,
  }) => {
  ...
  test("should have Add Patient and Assign Patient buttons", async ({
    page,
  }) => {
    await expect(
    ...
    await expect(
      page.getByRole("button", { name: /Assign Patient/i })
    ).toBeVisible();
  });

  test("should open Add Patient modal when Add Patient button is clicked", async ({
    page,
  }) => {
  ...
  test("should open Assign Patient modal when Assign Patient button is clicked", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Assign.*Patient/i })
    ).toBeVisible({ timeout: 10000 });
  });

  test("should filter patients when clicking filter buttons", async ({
    page,
  }) => {
    ...
    const highRiskButton = page.getByRole("button", {
      name: "High Risk",
      exact: true,
    });
...
test.describe("Patient Detail Page", () => {
  ...
  test("should display patient detail page when navigating to a patient", async ({
    page,
  }) => {
    ...
    if (patientId) {
      ...
      await expect(
        page.getByRole("button", { name: /View Questionnaire/i })
      ).toBeVisible({ timeout: 90000 });
  ...
  test("should display patient profile header information", async ({
    page,
  }) => {
    if (patientId) {
      await page.goto(`/patients/${patientId}`);
  ...
  test("should open questionnaire modal when View Questionnaire is clicked", async ({
    page,
  }) => {
    if (patientId) {
      await page.goto(`/patients/${patientId}`);
      ...
      const questionnaireButton = page.getByRole("button", {
        name: /View Questionnaire/i,
      });
      ...
      // Wait for modal to appear (adjust selector based on your modal implementation)
      await expect(page.getByText(/Questionnaire/i)).toBeVisible({
        timeout: 10000,
      });
    } else {
      test.skip();
  ...
  test("should display patient game scores section", async ({ page }) => {
    if (patientId) {
      ...
      await expect(
        page.getByRole("heading", { name: /Patient Game Scores/i })
      ).toBeVisible({ timeout: 90000 });
  ...
  test("should display test history table", async ({ page }) => {
    if (patientId) {
      ...
      await expect(
        page.getByRole("heading", { name: "Test History" })
      ).toBeVisible({ timeout: 90000 });
  ...
  test("should handle patient not found error gracefully", async ({ page }) => {
    ...
    const errorText = page.getByText(
      /Patient not found|Failed to load patient data/i
    );
  ...
  test("should navigate back to patients list from detail page", async ({
    page,
  }) => {
    if (patientId) {
      await page.goto(`/patients/${patientId}`);
...
test.describe("Patient Search and Filter Functionality", () => {
  ...
  test("should filter patients by search term", async ({ page }) => {
    ...
    const initialCount = await page
      .getByText(/Showing .* of .* patients/i)
      .textContent();
